...
```
gender seems to have the highest effect with the largest estimate 4.508675.
```
...
se
# get "Residual standard error" from summary
summary(mod)$sigma
# verify if they are match.
(se - summary(mod)$sigma) < 1e-8
```
...
# get p-values
t.test(women$los, men$los)$p.value
t.test(women$los, men$los, var.equal = T)$p.value
mod.c[2,4]
```
```
The second test gives the same p-value (0.0224) as seen in model summary.  
```
